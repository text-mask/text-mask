...
export default function createTextMaskInputElement(defaultConfig) {
  ...
  function update(rawValue, config = defaultConfig, isRecalculating = false) {
    const {
      inputElement,
      guide,
    ...
    let {pipe, mask: providedMask} = config

    // This is a solution to a problem with Android devices. When a user was typing fast or touched two buttons at once
    // just before a place where mask character should be added, caret did not maintain the correct position.
    // Thanks to this recalculation, caret is always in the right place on all Android devices.
    const recalculate = isRecalculating ? null : update.bind(this, undefined, config, true)
    ...
    safeSetSelection(inputElement, adjustedCaretPosition, recalculate) // adjust caret position
  }
