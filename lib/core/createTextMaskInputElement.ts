...
import { PipeResult } from '../types/Pipe';
...
export default function createTextMaskInputElement(config: MainConfig): TextMaskInputElementResult {
  // Anything that we will need to keep between `update` calls, we will store in this `state` object.
  const state = { previousConformedValue: '', previousPlaceholder: '' };
  ...
  return {
    ...
    update(
      ...
      if (piped && pipe) {
        ...
        if (isPipeResultsBoolean(pipeResults) && pipeResults === false) {
          // If the `pipe` rejects `conformedValue`, we use the `previousConformedValue`, and set `rejected` to `true`.
          pipeResults = { value: previousConformedValue, rejected: true };
        } else if (isPipeResultsString(pipeResults)) {
          pipeResults = { value: pipeResults };
        }
      }

      if (!isPipeResultsObject(pipeResults)) {
        throw new Error('Text-mask: createTextMaskInputElement: pipeResults should be an object');
      }
      ...
      inputElement.value = inputElementValue ?? EmptyString; // set the input value
      safeSetSelection(inputElement, adjustedCaretPosition ?? 0); // adjust caret position
    },
  };
}
...
function getSafeRawValue(inputValue: string | number): string {
