...
import { EmptyArray, EmptyString, PlaceholderChar } from '../utils/constants';
import {
  ...
  placeholderLoop: for (let i = 0; i < placeholderLength; i++) {
    ...
    if (charInPlaceholder === placeholderChar) {
      ...
      if (rawValueArr.length > 0) {
        ...
        while (rawValueArr.length > 0 && rawValueArr) {
          ...
          if (rawValueChar === placeholderChar && suppressGuide !== true) {
            ...
            // @ts-ignore
          } else if (mask[i] instanceof RegExp && mask[i].test(rawValueChar)) {
